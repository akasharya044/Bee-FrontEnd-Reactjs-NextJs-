...
const employeeSchema = z.object({
  ...
  address: z.string().min(1, 'Address is required').max(255, ' Address field should not accept more than 255 characters').optional(),
  contactNo: z.string()
...
pincode: z.string()
  ...
  .refine(isOnlyDigits, 'Pincode must contain only digits. any space,alphabets or special character not allowed.')
...
const EmployeeUpdate = (props:any) => {
  ...
  useEffect(() => {
   ...
   const storedValue = props.employeeId;
  ...
  const handleGetBYId = async (employeeId: any) => {
    
    try {
     
      const response = await apiservice.fetchData(
        `http://20.219.172.254/hiveconnect/accounts/clientemployee/employee/${employeeId}`
      );
  ...
  const getDesignationID=()=>{
    if(formData.designation){
      const desID=formData.designation
     
      const res=designation.find((item) => item.name === desID) ?? { id: '' };
    
      setdesignationOption(res?.id)
    }
  ...
  const getDepartmentID=()=>{
    if(formData.department){
      const depIDD=formData.department
      
      const res=department.find((item)=>item.name===depIDD)  ?? { id: '' };
     
      setdepartmentOption(res?.id)
    }
...
const handledesChange=(event:any)=>{
  ...
  const desIDD=event.target.value
  setdesignationOption(desIDD)  
  const depIDD=event.target.value
  
  }
  
  const handledeDepChange=(event:any)=>{    
    setdepartmentOption('') 
    const depIDD=event.target.value;
    setdepartmentOption(depIDD)
    }
